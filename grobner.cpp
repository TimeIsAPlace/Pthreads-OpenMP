...
void GE() {
	...
	cout << "Ordinary time:" << (tail - head) * 1000.0 / freq << "ms" << endl;
}
...
#pragma omp critical
	...
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	cout << "Omp time:" << (tail - head) * 1000.0 / freq << "ms" << endl;
}
...
void AVX_GE() {
	...
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	cout << "AVX time:" << (tail - head) * 1000.0 / freq << "ms" << endl;
}
...
#pragma omp critical
	...
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	cout << "AVX_omp time:" << (tail - head) * 1000.0 / freq << "ms" << endl;
}
...
void GE_pthread() {
	...
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	cout << "GE_pthread time:" << (tail - head) * 1000.0 / freq << "ms" << endl;
	free(handle);
	free(param);
...
void AVX_pthread() {
	...
	QueryPerformanceCounter((LARGE_INTEGER*)&tail);
	cout << "AVX_pthread time:" << (tail - head) * 1000.0 / freq << "ms" << endl;
	free(handle);
	free(param);
	pthread_mutex_destroy(&lock);
}
...
int main() {
		...
		ofstream out1("消元结果7(AVX).txt");
		ofstream out2("消元结果7(GE_pthreads).txt");
		ofstream out3("消元结果7(AVX_pthreads).txt");
		ofstream out4("消元结果7(GE_omp).txt");
		ofstream out5("消元结果7(AVX_omp).txt");
